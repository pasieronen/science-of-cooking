    ...
    var graph = (function () {
            var setup = function (div, data, flame, timestep, meatType, maxTemps, mode) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    for (var j = 0; j < data[0].length; j++) {
                        ...
                        if (state == nextState) {
                            ...
                            if (Math.abs(offset - state) > Math.abs(offset - nextState)) {
                                offset = boundaries[meatType].length * 2;
                            }
                ...
                var layers = stack(d3.range(n).map(function (d, i) {
                    return bumpLayer(i, dat);
                }));
                if (meatType == 'False') {
                    layers = stack(d3.range(data[0].length).map(function (d, i) {
                        return bumpLayer(i, data);
                    }));
                }
                ...
                if (meatType != 'False') {
                    ...
                    .data(function () {
                        return mode == 'C' ? CtempScale[meatType] : FtempScale[meatType]
                    })
                    ...
                    .style('fill', function (d) {
                        return color[meatType](d['position'])
                    ...
                    legend.append('text')
                        ...
                        .text(function (d, i) {
                            ...
                            if (i != 0) {
                                return (100 * (parseFloat(dat[59][6 - i]) + parseFloat(dat[59][6 + i])) / data[59].length).toFixed(0) + "% "
                            } else {
                                return (100 * (parseFloat(dat[59][(6 - i)])) / data[59].length).toFixed(0) + "% "
                            }
                        });
                ...
                var layer = svg.selectAll(".layer")
                    ...
                    .style("fill", function (d, i) {
                        return meatType == 'False' ? color[meatType](data[i] / tempMax) : color[meatType](i);
                    });
                ...
                var rect = layer.selectAll("rect")
                    ...
                    .attr("height", 0)
                    .style("fill", function (d, i) {
                        return meatType == 'False' ? color[meatType](data[i][d.y0] / tempMax) : ""
                    })
                ...
                imgstop.enter()
                    ...
                    .attr("xlink:href", function (d) {
                        return d[1] == 1 ? "flamedown.png" : "flame.png"
                    })
                    .attr("width", Math.min(x.rangeBand(), 30))
                    .attr("height", Math.min(x.rangeBand(), 30))
                    .attr("x", function (d) {
                        return (d[0] + 2) * (x.rangeBand() + 1)
                    })
                    .attr("y", function (d) {
                        return -Math.min(x.rangeBand(), 30) + (height + Math.min(x.rangeBand(), 30)) * (1 - d[1])
                    })
                ...
                linktext.enter().append("g").attr("class", "linklabelholder")
                    ...
                    .attr("dy", function (d) {
                        return -Math.min(x.rangeBand(), 30) / 1.2 + (height + 2 * Math.min(x.rangeBand(), 30)) * (1 - d[1])
                    })
                    ...
                    .text(function (d, i) {
                        ...
                        if (i == 0) {
                            return mode == 'C' ? d[2].toFixed(0) + "\xB0C" : toF(d[2]).toFixed(0) + "\xB0F";
                        }
